...
class Chain extends AsyncEmitter {
  ...
  async findFork(entry) {
    if (entry.height > this.height)
      entry = await this.getAncestor(entry, this.height);

    while (!await this.isMainChain(entry))
      entry = await this.getPrevious(entry);
   ...
   * is received.
   * @private
   * @param {ChainEntry} fork - Highest common ancestor.
   * @param {ChainEntry} new_ - The competing chain's tip.
   * @returns {Promise}
   */

  async reorganize(fork, new_) {
    const old = this.tip;
  ...
  async reorganizeSPV(competitor) {
    const tip = this.tip;
    const fork = await this.findFork(competitor);
  ...
  async setBestChain(entry, block, prev, flags) {
    ...
    if (!entry.prevBlock.equals(this.tip.hash)) {
      ...
      try {
        await this.reorganize(fork, await this.getPrevious(entry));
      } catch (err) {
        if (err.type === 'VerifyError') {
          if (this.tip.chainwork.lt(tip.chainwork))
            await this.reorganize(fork, tip);
        }
    ...
    try {
      ...
      if (err.type === 'VerifyError') {
        ...
        if (fork && this.tip.chainwork.lt(tip.chainwork))
          await this.reorganize(fork, tip);
      }
    ...
    if (fork) {
      ...
      await this.emitAsync('reorganize', tip, entry, fork);
    }
  ...
  async _add(block, flags, id) {
    ...
    if (flags & common.flags.VERIFY_BODY) {
      ...
      if (!valid) {
        const malleated = reason === 'bad-txnmrklroot'
                       || reason === 'bad-witnessroot';

        if (!malleated)
          this.setInvalid(block.hash());

        throw new VerifyError(block, 'invalid', reason, score, malleated);
      }
    }
