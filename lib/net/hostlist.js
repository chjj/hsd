...
class HostList {
  ...
  getLocal(dst) {
    ...
    if (!dst) {
      for (const src of this.local.values()) {
        if (this.network.type === 'main') {
          // Disable everything else for now.
          if (src.type < HostList.scores.UPNP)
            continue;
        }
    ...
    for (const src of this.local.values()) {
      ...
      const reach = src.addr.getReachability(dst);

      if (reach < bestReach)
        continue;

      if (reach > bestReach || src.score > bestScore) {
        bestReach = reach;
        bestScore = src.score;
        bestAddr = src.addr;
      }
    ...
    if (bestAddr)
      bestAddr.time = this.network.now();
